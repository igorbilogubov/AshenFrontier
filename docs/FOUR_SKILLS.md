...
| Mage | Цепная молния, `mage-lightning`: first target within 5.2 m forward, then up to two nearest unique targets within 1.9 m of the previous hit; 1.55× then 0.72× per jump, 30 MP, 7 s | Метеор, `mage-meteor`: server center 3.825 m forward, radius 1.9 m, up to 5 targets after 0.7 s; 1.72× center damage falling to 0.68× at edge, 38 MP, 12 s |

`hitFraction` and `durationScale` are explicit in shared skill definitions. Area damage follows cast contact only if center is walkable, outside camp and unobstructed. `skillImpact` for rain/meteor emits `phase:'warning'` with `delay` and `radius` at cast contact, then `phase:'impact'` at server damage time, both at the same authoritative center. Lightning emits one event per actual unique chain target with `from` pointing to the caster or prior target. Projectile contact emits the existing `skillImpact`; no client particle causes damage. A cancelled AFK action cannot land a pending area hit. Pending areas are world memory and do not resume after server restart; cooldown and mana are saved.

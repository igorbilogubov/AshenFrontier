...
type Clip=typeof CLIP_NAMES[number];
...
const lights: [position:[number,number,number],color:string,intensity:number][]=[[[3,5,4],'#ffe8be',3],[[-4,2,2],'#aecae9',2],[[2,4,-3],'#c7d9eb',3]];
for(const [pos,color,intensity] of lights){const l=new T.DirectionalLight(color,intensity);l.position.set(...pos);scene.add(l);if(pos[0]===3){l.castShadow=true;l.shadow.mapSize.set(1024,1024);Object.assign(l.shadow.camera,{left:-3,right:3,top:3,bottom:-3});l.shadow.normalBias=.015;}}
...
let warrior:Warrior,clip:Clip='Idle',elapsed=0,paused=false,last=0;
...
function select(name:Clip){clip=name;elapsed=0;warrior.previewClip(name);for(const b of $('clips').querySelectorAll<HTMLButtonElement>('button'))b.setAttribute('aria-pressed',String(b.dataset.clip===name));}
function status(){const duration=warrior.clips[clip].duration;$('timeline').value=String(Math.round(Math.min(elapsed/duration,1)*1000));$('status').textContent=`${labels[CLIP_NAMES.indexOf(clip)]} · ${Math.min(elapsed,duration).toFixed(2)} / ${duration.toFixed(2)} с${paused?' · пауза':''}`;}
try{
  ...
  select('Idle');
  $('weapon').onchange=()=>warrior.equipment($('weapon').value==='axe'?'axe':'sword');

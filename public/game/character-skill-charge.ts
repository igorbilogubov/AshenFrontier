...
export function createCharacterSkillCharge(model:T.Object3D){
  ...
  return {update(hero:WarriorPose){
    ...
    const frost=skill==='mage-frost'||skill==='mage-ice-lance'||skill==='mage-ice-step',lightning=skill==='mage-lightning'||skill==='mage-beam'||skill==='mage-teleport'||skill==='mage-ward'||skill==='mage-mana-source'||skill==='mage-arcane-nova',meteor=skill==='mage-meteor',warrior=skill.startsWith('warrior');
    ...
    ring.visible=frost||meteor||skill==='warrior-shockwave'||skill==='warrior-earthquake'||skill==='mage-arcane-nova';ring.scale.setScalar((frost?.8:meteor?.5:skill==='mage-arcane-nova'?1:.38)*power);ring.rotation.z=p*4;ring.material.opacity=power*.55;

...
export function createAnimatedWarrior(gltf:{scene:T.Object3D;animations:T.AnimationClip[]},classId:ClassId='warrior',combatClips:T.AnimationClip[]=[],lateSource?:T.Object3D){
  ...
  function animate(dt:number,hero:WarriorPose,sampleAnimation=true){
    ...
    lateVisuals.applyEnhancement(hero.enhances??hero.enhance??0);
  ...
  return {root,model,mixer,clips,animate,equipment,previewClip,samplePreview,applyEnhancement:lateVisuals.applyEnhancement,disposeExtras:()=>{bowPresentation.dispose();skillCharge.dispose();lateVisuals.dispose();},

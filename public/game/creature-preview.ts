...
const lights: [position:[number,number,number],color:string,intensity:number][]=[[[3,5,4],'#ffe8be',3],[[-4,2,2],'#aecae9',2],[[2,4,-3],'#c7d9eb',2]];
for(const [pos,color,intensity] of lights){const l=new T.DirectionalLight(color,intensity);l.position.set(...pos);scene.add(l);if(pos[0]===3){l.castShadow=true;l.shadow.mapSize.set(1024,1024);Object.assign(l.shadow.camera,{left:-4,right:4,top:4,bottom:-4});l.shadow.normalBias=.012;}}
...
const labels:Record<string,string>={Idle:'Покой',Walk:'Шаг',Run:'Бег',Attack:'Атака',Hit:'Ранение',Death:'Падение',Turn_Left:'Влево',Turn_Right:'Вправо'};
let models:Partial<Record<CreatureKey,MobModel>>={},current:MobModel,beforePromise:Promise<void>|null=null,clip='Idle',elapsed=0,paused=false,last=0,selectionRequest=0;
...
function selectClip(name:string){clip=name;elapsed=0;current.previewClip(name);for(const b of $('clips').querySelectorAll<HTMLButtonElement>('button'))b.setAttribute('aria-pressed',String(b.dataset.clip===name));}
async function selectCreature(type:MobType){
  ...
  const selected=models[key];if(!selected)throw new Error(`Missing creature: ${key}`);current=selected;$('creature-name').textContent=MOB_TYPES[type].name;
...
try{
  const assets=await loadMobAssets();models=Object.fromEntries((Object.keys(MOB_TYPES) as MobType[]).map(type=>{const model=createMob(type,assets);scene.add(model.root);return [type,model];}));
  await selectCreature('wolf');$('creature').onchange=$('version').onchange=()=>selectCreature(creatureType($('creature').value));
...
}catch(error){$('status').textContent=errorMessage(error);console.error(error);}

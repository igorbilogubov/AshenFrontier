...
function preset(kind:'wanderer'|'watch'|'none'){
  ...
  if(kind!=='none')for(const definition of WARRIOR_ITEMS)if(definition.id.startsWith(kind)||definition.slot==='ring'||definition.slot==='amulet')equipment[definition.slot]=samples.get(definition.id)!.id;

...
export function bindInterface(game,toast,clearInput){
  let inventoryKey='',statsKey='',selectedItemId=null,draft=emptyDraft(),draftOwner='',pending=null,statusMessage='',lastPanel='character';
  const compact=matchMedia('(max-width: 900px)'),panels={character:$('character-panel'),inventory:$('inventory-panel')};
  const statNodes=new Map(),derivedNodes=new Map(),slotNodes=new Map(),bagNodes=[];
  ...
  function closePanel(name){panels[name].hidden=true;syncPanels();if(!isPanelOpen())$('scene').focus({preventScroll:true});}
  function togglePanel(name){if(panels[name].hidden)openPanel(name);else closePanel(name);}
  ...
  for(const key of STAT_KEYS){
    ...
    for(const [button,amount] of [[minus,-1],[plus,1]]){
  ...
  function sendStatCommand(type,extra={}){
    if(pending||!canEdit())return;
    pending={revision:game.player.statRevision,classId:game.player.classId,sentAt:performance.now()};statusMessage='';
    game.send({type,revision:pending.revision,...extra});statsKey='';$('reset-confirm').hidden=true;updateStats();
  }
  ...
  function setIcon(element,slot,classId){const key=slot+':'+classId;if(element.dataset.icon!==key){element.innerHTML=itemIcon(slot,classId);element.dataset.icon=key;}}
  ...
  function updateStats(){
    ...
    const current=characterStats(p),proposed=Object.fromEntries(STAT_KEYS.map(stat=>[stat,(p.allocatedStats?.[stat]||0)+draft[stat]]));

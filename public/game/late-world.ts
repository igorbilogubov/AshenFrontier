...
export const LATE_MOB_TYPES:Readonly<Record<LateMobType,LateMobConfig>>=Object.freeze({
 'swamp-frog':cfg('Топяная жаба',40,1400,62,1.7,.58,892),
 'marsh-crocodile':cfg('Болотный крокодил',44,1700,68,1.5,.75,1077),
 'plague-mosquito':cfg('Чумной комар',49,2050,75,2.3,.46,1333),
 'bog-spider':cfg('Топяной паук',54,2400,82,1.8,.7,1616),
 'cave-bat':cfg('Пещерный нетопырь',55,2300,80,2.35,.48,1676),
 'cave-crawler':cfg('Пещерный ползун',59,2700,88,1.65,.68,1927),
 'crystal-beetle':cfg('Кристальный жук',64,3100,96,1.45,.78,2265),
 'stone-guardian':cfg('Каменный страж',69,3500,105,1.25,.88,2631),
 hellhound:cfg('Адская гончая',70,3500,105,2.4,.53,2707),
 'lava-elemental':cfg('Лавовый элементаль',74,4000,114,1.45,.78,3024),
 'ember-crab':cfg('Угольный краб',79,4500,124,1.6,.87,3445),
 'basalt-brute':cfg('Базальтовый громила',84,5000,135,1.2,.94,3893),
 bonehound:cfg('Костяная гончая',85,5000,135,2.45,.55,3986),
 gargoyle:cfg('Горгулья',89,5650,148,1.75,.8,4369),
 'void-stalker':cfg('Ловчий пустоты',94,6300,161,2.1,.67,4872),
 'iron-warden':cfg('Железный надзиратель',99,7000,175,1.3,.95,5403),
});
...
export const LATE_REGIONS:readonly LateRegion[]=Object.freeze(DESIGNS.map((d,index)=>{
 ...
 return {id:d.id,name:d.name,minLevel:d.minLevel,bounds:{minX:x,maxX:x+180,minZ:-90,maxZ:90},entry,safe,dungeonEntrance,spawns,spots,roads,props,obstacles:[...props.map(v=>({x:v.x,z:v.z,r:v.r})),...[-1,1].map(side=>({x:dungeonEntrance.x+side*2.4,z:dungeonEntrance.z,w:1.2,d:1.5}))],passages};
}));

...
export interface HeldOptions {held?:boolean}

// A live direction, never a destination retained after the button is released.
export function heldMouseInput(hero:MouseHero,point:Point|null,{held=false}:HeldOptions={}):HeldInput{
  ...
  const strength=Math.min(1,(distance-.18)/.7);

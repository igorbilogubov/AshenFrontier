import {CLASS_ITEMS,rollEquipment} from './equipment-items.js';
import {MOB_TYPES} from './location.js';
import {GEAR_CHANCE} from './loot-rules.js';
...
export function possibleLoot(type:MobType):PossibleLoot{
  ...
  for(const id of ['weapon','armor','accessory'] as const){
    const slots=GROUPS[id],definitions=Object.values(CLASS_ITEMS).flat().filter(item=>slots.includes(item.slot));
    ...
    for(const rarity of [...rarities].sort((a,b)=>a-b))categories.push({id,name:LABELS[id],rarity,slots});
  }
  return {gold:MOB_TYPES[type].coins,itemChance:GEAR_CHANCE[type],categories};
}

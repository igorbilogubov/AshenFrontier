...
type ImpactEvent=EventPayloads['skillImpact']&{phase?:'warning'|'impact';delay?:number;radius?:number;from?:Point};
...
export function createSkillEffects(scene:T.Scene){
  ...
  function sample(effect:typeof effects[number]){
    ...
    const lightning=skill==='mage-lightning',rain=skill==='archer-rain',meteor=skill==='mage-meteor',ice=skill==='mage-frost'||skill==='archer-frost-shot';
  ...
  return {
    impact(event:ImpactEvent,elapsedSeconds=0){
      const effect=effects[cursor++%effects.length],skill=SKILLS[event.skillId];if(!skill)return;
      ...
      effect.radius=event.radius??skill.radius??(skill.classId==='archer'?.6:skill.range);effect.active=true;
      ...
      effect.ring.rotation.set(-Math.PI/2,0,event.skillId.startsWith('warrior-')?-Math.PI/2:0);effect.ring.visible=event.skillId!=='mage-lightning'&&event.skillId!=='warrior-thrust';

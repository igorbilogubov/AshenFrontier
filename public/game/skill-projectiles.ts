...
export function createSkillProjectile(p:PublicProjectile){
  const group=new T.Group(),skill:string=p.skillId||'',arrow=p.kind==='archer',frost=skill==='archer-frost-shot',fire=skill==='mage-fireball',piercing=skill==='archer-piercing';
  const color=frost?'#a1e8ff':fire?'#ff8d36':arrow?piercing?'#ffecaa':'#d9c89f':'#b9a4ed';
  const shaft=new T.Mesh(arrow?new T.CylinderGeometry(.017,.017,piercing?1.05:.65,5):new T.IcosahedronGeometry(fire?.22:.13,1),new T.MeshBasicMaterial({color}));
  if(arrow){shaft.geometry.rotateX(Math.PI/2);const tip=new T.Mesh(new T.ConeGeometry(frost?.08:.05,.16,4),new T.MeshBasicMaterial({color:frost?'#d5fbff':'#e9e4ce'}));tip.rotation.x=Math.PI/2;tip.position.z=piercing?.59:.39;group.add(tip);
    const feather=new T.Mesh(new T.PlaneGeometry(.15,.18),new T.MeshBasicMaterial({color:skill==='archer-volley'?'#9fcb88':'#a7a090',side:T.DoubleSide}));feather.rotation.x=Math.PI/2;feather.position.z=-.24;group.add(feather);
  }
  group.add(shaft);
  if(p.skillId){

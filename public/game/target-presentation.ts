...
import type {DungeonId,MobType} from '../../shared/types.js';

export type PresentedTarget =
  | {kind:'mob';type:MobType;eliteId?:string;bossId?:DungeonId;dungeonId?:DungeonId;x:number;z:number;hp:number;maxHp:number;name:string}
...
const RARITY_NAMES=['Обычный','Необычный','Редкий','Возвышенный','Сетовый'];
...
export function bindTargetPresentation(scene:T.Scene):(target:PresentedTarget|null)=>void{
  ...
  return (target)=>{
    ...
    panel.hidden=false;name.textContent=target.name;panel.classList.toggle('elite-target',target.kind==='mob'&&!!target.eliteId);panel.classList.toggle('boss-target',target.kind==='mob'&&!!target.bossId);

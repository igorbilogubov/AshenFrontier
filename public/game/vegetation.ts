...
function trianglesGeometry(triangles:Triangle[],colors?:number[]){
...
export function pineGeometry(){
  const triangles:Triangle[]=[],colors:number[]=[];
  const add=(a:Vertex,b:Vertex,c:Vertex,shade:number)=>{triangles.push([a,b,c]);const color=new T.Color('#4d6849').multiplyScalar(shade);for(let i=0;i<3;i++)colors.push(color.r,color.g,color.b);};
  for(let tier=0;tier<8;tier++)for(let branch=0;branch<8;branch++){
    ...
    const p=(t:number,side=0,h=0):Vertex=>[Math.cos(a)*t*r-Math.sin(a)*side,y+.15-.32*t+h,Math.sin(a)*t*r+Math.cos(a)*side];
...
export function grassGeometry(){
  ...
  for(let i=0;i<9;i++){
    ...
    const p=(y:number,b:number,s:number):Vertex=>[x+Math.sin(a)*b+Math.cos(a)*s,y,z+Math.cos(a)*b-Math.sin(a)*s];
...
export function fernGeometry(){
  ...
  for(let frond=0;frond<7;frond++){
    ...
    const p=(t:number,side:number):Vertex=>[Math.sin(a)*t*.60*scale+Math.cos(a)*side,Math.sin(t*Math.PI*.86)*.37,Math.cos(a)*t*.60*scale-Math.sin(a)*side];
    for(let i=1;i<9;i++){
...
export function windMaterial(clock:T.IUniform<number>,strength=.02){

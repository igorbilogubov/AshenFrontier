...
export function createWastelandEnvironment(scene:T.Scene){
 ...
 const geometry=new T.PlaneGeometry(188,188,144,144);geometry.rotateX(-Math.PI/2);geometry.translate(600,-.045,0);const colors:number[]=[],p=geometry.attributes.position;
 ...
 for(const mark of WASTELAND_LANDMARKS){const g=joint(statics,mark.x,0,mark.z);
  if(mark.kind==='pillars'){for(const dx of [-2.5,0,2.5]){cylinder(g,.38,.55,4.3-Math.abs(dx)*.3,basalt,dx,2.1-Math.abs(dx)*.15,0,6);cylinder(g,.63,.52,.35,sand,dx,4.1-Math.abs(dx)*.3,0,6);}}
  if(mark.kind==='tomb'){box(g,5,1.3,3,basalt,0,.65,0);box(g,5.25,.24,3.25,sand,0,1.42,0);for(const dx of [-1.6,0,1.6])cylinder(g,.24,.34,.6,bronze,dx,1.82,0,6);}

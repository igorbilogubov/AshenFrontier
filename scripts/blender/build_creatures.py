...
def create_rig(kind):
    ...
    for side in [-1,1]:
        ...
        for front_leg in [True,False]:
            ...
            elbow=(x,.48 if kind=='wolf' else .40 if boar else .46,z-(.22 if kind=='wolf' else .12)) if front_leg else (x,.44 if boar else .53,z+.22)
...
def fur_colors(obj,kind,detail=False):
    ...
    if kind=='wolf':dark=Vector((.045,.061,.067));mid=Vector((.19,.23,.235));light=Vector((.48,.46,.39))
    for v in obj.data.vertices:
        x,y,h=v.co;z=-y
        ...
        if kind=='wolf':
            saddle=smooth((h-.84)/.18)*(1-smooth((z-.38)/.38))
            cheeks=smooth((z-.51)/.2)*(1-smooth((h-1.13)/.10))
            c=mid.lerp(dark,max(saddle*.95,top*.60)).lerp(light,max(belly*.85,cheeks*.75))
        c*=1+n*.18+fine*.10
        if detail:c*=.92
...
def build_model(kind,arm):
    ...
    if boar:
        ...
        for c,r in [((0,.83,-.46),(.205,.225,.29)),((0,.87,-.14),(.185,.18,.37)),((0,.92,.16),(.245,.265,.36)),((0,1.015,.36),(.21,.275,.24)),((0,1.105,.51),(.18,.23,.205)),((0,1.165,.66),(.165,.135,.19)),((0,1.11,.88),(.088,.076,.225)),((0,1.10,1.05),(.064,.051,.073))]:s.ellipsoid(c,r)
        for side in [-1,1]:
            s.ellipsoid((side*.165,.75,-.40),(.12,.20,.18))
            s.ellipsoid((side*.16,.85,.30),(.115,.22,.15))
    else:
    ...
    for suffix in ['L','R']:
        for front in [True,False]:
            ...
            if kind=='wolf':
                s.tube([a,(foot[0],.060,foot[2]+.018)],[.041,.053])
                s.ellipsoid((foot[0],.047,foot[2]+.025),(.065,.041,.093 if front else .085))
            else:s.ellipsoid((foot[0],.072,foot[2]+.035),(.073 if boar else .064,.062,.115 if front else .10))
    if not boar:
    ...
    jaw=Sculpt();jaw.ellipsoid((0,.43 if boar else 1.018 if kind=='wolf' else 1.005,.81),(.15 if boar else .095 if kind=='wolf' else .105,.075 if boar else .043,.245 if boar else .18))
    ...
    for side,suffix in [(-1,'L'),(1,'R')]:
        ...
        e=Sculpt();e.ellipsoid((side*(.245 if boar else .125 if kind=='wolf' else .147),.79 if boar else 1.197,.69 if boar else .76),(.018 if kind=='wolf' else .024,.014 if kind=='wolf' else .018,.030 if kind=='wolf' else .038));detail('Eye_Socket_'+suffix,e,dark,'Head')
        e=Sculpt();e.ellipsoid((side*(.264 if boar else .142 if kind=='wolf' else .167),.794 if boar else 1.198,.707 if boar else .776),(.006 if kind=='wolf' else .008,.007 if kind=='wolf' else .009,.011 if kind=='wolf' else .013));detail('Eye_'+suffix,e,eyes,'Head')
        pupil=Sculpt();pupil.ellipsoid((side*(.270 if boar else .147 if kind=='wolf' else .173),.796 if boar else 1.199,.713 if boar else .780),(.004,.006 if kind=='wolf' else .007,.006));detail('Pupil_'+suffix,pupil,dark,'Head')
        teeth=Sculpt()
        if boar:
        ...
        for front in [True,False]:
            ...
            for offset in ([-.036,.036] if boar else [-.035,0,.035]):
                ...
                else:nails.tube([(paw.x+offset,.034 if kind=='wolf' else .047,-paw.y+.095),(paw.x+offset,.019 if kind=='wolf' else .025,-paw.y+(.121 if kind=='wolf' else .142))],[.009 if kind=='wolf' else .012,.0015],6)
            detail('Claws_'+name,nails,dark if boar or kind=='wolf' else ivory,name+'_Paw' if front else name+'_Toe')
    ...
    if boar:
        ...
        for i in range(42 if alpha else 16):
            ...
            ruff.tube([(x*.95,h,z),(x*1.04,h-.025,z-.045),(x*1.10,h-.055,z-length)],[.028 if alpha else .040,.017,.0015],6)
...
def solve_leg(arm,prefix,forward,lift,front,plant_x=False,lateral=0,paw_pitch=0,paw_yaw=0):
    ...
    target.x=target.x+lateral if plant_x else hip.x
    ...
    foot_rotation=Quaternion((0,0,1),paw_yaw)@Quaternion((1,0,0),paw_pitch)
    tail=target+foot_rotation@(paw.bone.tail_local-paw.bone.head_local)
    aim_bone(arm,prefix+'_Paw',target,tail)
    if not front:
        toe=arm.pose.bones[prefix+'_Toe'];aim_bone(arm,prefix+'_Toe',tail,tail+foot_rotation@(toe.bone.tail_local-toe.bone.head_local))

def refine_wolf_shape(arm,body):
    def warp(point):
        p=point.copy();height=p.z;forward=-p.y
        head=smooth((forward-.28)/.38)*smooth((height-.72)/.32)
        p.z-=.105*head+.18*max(0,height-1.27)
        return p
    for vertex in body.data.vertices:vertex.co=warp(vertex.co)
    bpy.context.view_layer.objects.active=arm
    bpy.ops.object.mode_set(mode='EDIT')
    for bone in arm.data.edit_bones:bone.head=warp(bone.head);bone.tail=warp(bone.tail)
    bpy.ops.object.mode_set(mode='OBJECT')
    body.data.update();bpy.context.view_layer.update()

def wolf_locomotion(arm,clip,t):
    """Four-beat lateral walk, diagonal trot, and planted-foot pivot steps."""
    phase=t*TAU;run=clip=='Run';turn=clip.startswith('Turn_');direction=1 if clip=='Turn_Left' else -1
    root=arm.pose.bones['Root']
    # Small weight transfer; IK below keeps supporting paws fixed in world space.
    sway=(.007 if run else .014)*math.sin(phase)
    bounce=(.024 if run else .006)*math.cos(phase*2)
    root.location=root.bone.matrix_local.to_3x3().inverted()@V(sway,bounce,0)
    rotate_world(arm,'Hips',(.010*math.sin(phase),0,.012*math.sin(phase)))
    rotate_world(arm,'Chest',(-.012*math.sin(phase),0,-.010*math.sin(phase)))
    rotate_world(arm,'Neck',(.014*math.sin(phase+.4),0,0))
    rotate_world(arm,'Head',(-.014*math.sin(phase+.4),0,.006*math.sin(phase)))
    rotate_world(arm,'Tail_1',(.015,0,.04*math.sin(phase+.4)))
    rotate_world(arm,'Tail_2',(.025*math.sin(phase+.2),0,.025*math.sin(phase-.2)))
    bpy.context.view_layer.update()
    offsets={'Front_L':0,'Front_R':.5,'Hind_L':.5 if run else .25,'Hind_R':0 if run else .75}
    for prefix,offset in offsets.items():
        front=prefix.startswith('Front');p=(t+offset)%1;stance=.56 if run else .64;stride=1.12 if run else .72
        span=stride*stance;lift=0;swing=0
        if p<stance:forward=span/2-stride*p
        else:
            swing=(p-stance)/(1-stance)
            # Match horizontal velocity at lift-off and touchdown, unlike a
            # smoothstep whose zero endpoint speed caused a sudden paw jerk.
            forward=mix(-span/2,span/2,smooth(swing))-stride*(1-stance)*swing*(1-swing)*(1-2*swing)
            lift=math.sin(swing*math.pi)**2*(.125 if run else .073)
        lateral=0;paw_yaw=0
        if turn:
            angle=forward/stride*1.0*direction
            ankle=arm.data.bones[prefix+'_Lower'].tail_local
            rotated=Quaternion((0,0,1),angle)@ankle
            lateral=rotated.x-ankle.x;forward=ankle.y-rotated.y;paw_yaw=angle
            lift*=.85
        solve_leg(arm,prefix,forward,lift,front,True,lateral,.16*math.sin(math.pi*swing)**2,paw_yaw)
...
def pose(arm,kind,clip,t):
    ...
    if kind=='wolf' and clip in ['Walk','Run','Turn_Left','Turn_Right']:
        wolf_locomotion(arm,clip,t);return
...
def animate(arm,kind):
    ...
    clips=[('Idle',2.4),('Walk',1.2),('Run',.8),('Attack',1.0),('Hit',.5),('Death',1.4)]
    if kind=='wolf':clips.extend([('Turn_Left',1.2),('Turn_Right',1.2)])
...
def build(kind):
    bpy.ops.wm.read_factory_settings(use_empty=True);scene=bpy.context.scene;arm=create_rig(kind);body=build_model(kind,arm)
    if kind=='wolf':refine_wolf_shape(arm,body)
    actions=animate(arm,kind)

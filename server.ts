...
const economy=(p:PersistentHero)=>JSON.stringify([p.gold,p.xp,p.level,p.kills,p.items,p.pendingItems,p.stash,p.equipment,p.consumableInventory,p.quickSlots,p.consumableOverflow,p.allocatedStats,p.statRevision,p.potions,p.manaPotions,p.questKills,p.questClaimed,p.boss,p.afkPreferences,p.skillBuild,p.buildRevision,p.skillPresets]);

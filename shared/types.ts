...
export type PublicPlayer = Pick<Hero, 'id' | 'name' | 'classId' | 'x' | 'z' | 'yaw' | 'weapon' | 'hp' | 'level' | 'dead' | 'hurt' | 'attack' | 'moveBlend' | 'runBlend' | 'gait' | 'vx' | 'vz' | 'connected'> & { maxHp: number; effects?:SkillEffect[]; appearance?:ItemAppearance; enhance?:number; enhances?:Partial<Record<EquipmentSlot,number>> };
export type OnlinePlayer = Pick<Hero,'id'|'name'|'classId'|'level'> & {location:LocationId};
export type SelfSnapshot = PersistentHero & {navigationTarget?:Point|null;attackTargetId?:number|null;travelPortalId?:string;campReturnRemaining?:number;appearance?:ItemAppearance;enhance?:number;enhances?:Partial<Record<EquipmentSlot,number>>;afk?:AfkState|null;afkRadius?:number;afkXpMinute?:number;interactionTarget?:InteractionTarget|null;shopActive?:boolean;stashActive?:boolean;smithActive?:boolean} & Omit<CharacterStats, 'attack'> & Pick<Hero, 'targetYaw' | 'vx' | 'vz' | 'hurt' | 'gait' | 'moveBlend' | 'runBlend' | 'ack'>;

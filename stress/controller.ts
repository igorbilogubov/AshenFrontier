...
interface Bot {socket:WebSocket;id:string;seq:number;state:SelfSnapshot|null;index:number;nextSkill:0|1}
...
class StressController {
  ...
  private async addBot(url:string,index:number,warriorsOnly=false){
    const bot:Bot={socket:new WebSocket(url.replace('http:','ws:')+'/ws',{origin:url}),id:'',seq:0,state:null,index,nextSkill:0};
  ...
  private async scenario(players:number,mode:'camp'|'combat'|'afk',url:string,equipment:unknown){
    ...
    let index=0,playerIndex=0;for(const hero of participants){
      ...
      if(equipment==='mixed-classes'){
        ...
        hero.items=definitions.map(definition=>({...rollEquipment(definition.id,oldId+'-'+definition.id,()=>.5),bound:true}));
        hero.equipment={};

...
async function connect(server,join){
  ...
  const c={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url,headers:login.headers}),states:[]};
...
test('real online AFK stops with the socket and never resumes from saved V3 after restart',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase(),fixture='e'.repeat(48),anchor={x:7.6,z:1.8},hero=newHero('Сетевая опушка','warrior');
  Object.assign(hero,anchor,{level:35});hero.hp=stats(hero).maxHp;hero.mana=stats(hero).maxMana;assert.equal(afkSpotAt(hero),null);await database.seed(fixture,persistentHero(hero));
  let server;const clients=[];
  try{
    server=await startTestServer(database);const owner=await connect(server,{fixture}),observer=await connect(server,{name:'Наблюдатель',classId:'mage'});clients.push(owner,observer);
    ...
    const saved=(await database.load(fixture)).hero;assert(!('afk' in saved));
...
test('real online AFK collects personal gold and PostgreSQL keeps the award after restart',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase(),fixture='f'.repeat(48),spot=AFK_SPOTS[0],hero=newHero('Автосбор','warrior');
  Object.assign(hero,{x:spot.x,z:spot.z,level:35});hero.hp=stats(hero).maxHp;hero.mana=stats(hero).maxMana;
  await database.seed(fixture,persistentHero(hero));let server;const clients=[];
  try{
    server=await startTestServer(database);const owner=await connect(server,{fixture});clients.push(owner);
    ...
    let durable=0;
    for(let i=0;i<40&&durable<awarded;i++){durable=(await database.load(fixture)).hero.gold;if(durable<awarded)await delay(100);}
...
test('network AFK enables quietly in town for every class and publishes only the server activation point',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    const heroes=['warrior','archer','mage'].map((classId,index)=>({hero:newHero(`Лагерь ${index}`,classId),fixture:String(index+1).repeat(48)}));
    for(const {hero,fixture} of heroes){Object.assign(hero,anchor);await database.seed(fixture,persistentHero(hero));}
    server=await startTestServer(database);
    for(const {fixture} of heroes){
      const client=await connect(server,{fixture});clients.push(client);

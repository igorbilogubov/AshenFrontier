...
async function connect(server,token){
  const c={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url}),state:null};
...
test('WebSocket potion assignment, purchase and use are durable before their snapshot and survive restart',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase(),token='b'.repeat(48),hero=newHero('Бутылки');
  Object.assign(hero,{x:SHOP.x,z:SHOP.z,gold:100,hp:1,mana:0});
  await database.seed(token,persistentHero(hero));let server,c;
  try{
    server=await startTestServer(database);c=await connect(server,token);
    ...
    assert.deepEqual((await database.load(token)).hero.quickSlots,c.state.self.quickSlots);
    ...
    const used=await database.load(token);assert.equal(used.hero.potions,2);assert.equal(used.hero.manaPotions,2);
    ...
    const bought=await database.load(token);assert.equal(bought.hero.gold,94);assert.deepEqual(bought.hero.consumableInventory,c.state.self.consumableInventory);
    ...
    await close(c);c=null;await stopTestServer(server);server=await startTestServer(database);c=await connect(server,token);

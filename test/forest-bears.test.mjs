...
test('bear grove online AFK keeps the hero in its own ring and drops manual loot',()=>{
  ...
  assert(hero.kills>0);assert.equal(hero.gold,0);assert.equal(hero.questKills,0);
  assert(world.groundLoot.some(drop=>drop.owner===hero.id));
});

...
async function connect(server,token){
  const client={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url}),state:null,events:[]};
  ...
  await once(client.ws,'open');client.send=message=>client.ws.send(JSON.stringify(message));client.send({type:'join',protocol:2,token});
...
test('personal ground gold needs a server pickup and persists atomically, while unpicked drops vanish on restart',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    const token='d'.repeat(48);await database.seed(token,persistentHero(hero));
    server=await startTestServer(database,{dataDir});const client=await connect(server,token);clients.push(client);
    ...
    const dbBefore=await database.load(token);assert.equal(dbBefore.hero.gold,0);assert(dbBefore.hero.xp>0);
    ...
    assert.equal((await database.load(token)).hero.gold,8);

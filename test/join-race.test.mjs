...
async function connect(server,claim){
  const client={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url}),state:null,error:null,welcome:null,paused:false};
  ...
  await once(client.ws,'open');client.ws.send(JSON.stringify({type:'join',protocol:2,...claim}));
  return client;
}
...
test('join in flight gets explicit storage_unavailable when a slow durable save starts',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase(),token='a'.repeat(48),hero=newHero('Race owner');Object.assign(hero,{hp:40,x:5,z:12});
  ...
  try{
    ...
    owner=await connect(server,{token});await until(()=>owner.state,{message:'Owner did not join'});
    ...
    const stored=await admin.query('SELECT count(*)::integer AS count FROM heroes');assert.equal(stored.rows[0].count,1);

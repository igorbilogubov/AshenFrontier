...
async function connect(server,join={}){
  ...
  while(Date.now()<deadline){
    const c={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url,headers:login.headers}),history:[],events:[]};
    ...
    try{
      ...
      c.send({type:'join',protocol:3,heroId:login.heroId});
...
test('real WebSockets share movement/boss rewards and keep identity, loot and cooldowns through disconnect and restart',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    for(const [fixture,hero] of [[keys[0],a],[keys[1],b],[keys[2],third]])await database.seed(fixture,persistentHero(hero));
    server=await startTestServer(database);
    const ca=await connect(server,{fixture:keys[0]}),cb=await connect(server,{fixture:keys[1]});clients.push(ca,cb);
    await until(()=>ca.state.players.length===2&&cb.state.players.length===2);
    const duplicate=await connect(server,{fixture:keys[0]});clients.push(duplicate);assert.equal(duplicate.error.code,'in_use');
    const bad=await connect(server,{fixture:'invented'});clients.push(bad);assert.equal(bad.error.code,'invalid_character');
    ...
    await close(ca);const rejoined=await connect(server,{fixture:keys[0]});clients.push(rejoined);
    ...
    const thirdClient=await connect(server,{fixture:keys[2]});clients.push(thirdClient);assert.equal(thirdClient.state.self.classId,'archer');assert.equal(thirdClient.state.self.gold,2189);assert.equal(thirdClient.state.self.level,10);assert.equal(thirdClient.state.self.xp,317);
    ...
    const restored=await connect(server,{fixture:keys[0]});clients.push(restored);
    for(const k of ['id','gold','xp','items','boss'])assert.deepEqual(restored.state.self[k],earned[k],k);
    const oldAgain=await connect(server,{fixture:keys[2]});clients.push(oldAgain);assert.equal(oldAgain.state.self.id,identity);assert.equal(oldAgain.state.self.gold,2189);assert.equal(oldAgain.state.self.level,10);
    assert.equal(new Set((await Promise.all(keys.map(fixture=>database.load(fixture)))).map(row=>row.hero.id)).size,3);
...
test('stat allocation is atomic over WebSocket and restores a saved archer from PostgreSQL',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase();let server;const clients=[];
  try{
    const savedHero=newHero('Наследник','archer');
    Object.assign(savedHero,{level:7,xp:173,gold:842,questKills:5,boss:true,questClaimed:true});
    const saved=persistentHero(savedHero);
    const fixture='d'.repeat(48);
    await database.seed(fixture,saved);
    server=await startTestServer(database);
    const owner=await connect(server,{fixture,classId:'mage'}),observer=await connect(server,{name:'Наблюдатель',classId:'mage'});clients.push(owner,observer);
    ...
    await close(owner);
    const rejoined=await connect(server,{fixture,classId:'warrior'});clients.push(rejoined);
    ...
    await stopTestServer(server);server=await startTestServer(database);
    const restored=await connect(server,{fixture,classId:'mage'});clients.push(restored);
    ...
    const disk=(await database.load(fixture)).hero;
...
test('rolled equipment survives real WebSocket equip, observer updates and a server restart',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    server=await startTestServer(database);const owner=await connect(server,{fixture:keys[0]}),watcher=await connect(server,{fixture:keys[1]});clients.push(owner,watcher);

...
test('skillbook catalog presents twelve level-ordered skills and three complete talent branches per class',()=>{
  for(const classId of classes){
    const skills=skillsForClass(classId);assert.equal(skills.length,12,classId);assert.deepEqual(skills.map(skill=>skill.unlockLevel),[1,3,5,8,10,14,18,22,27,32,38,45]);
...
test('all twenty-four new actions have a distinct authored HUD silhouette',()=>{
  assert.equal(newSkills.length,24);const icons=newSkills.map(skill=>actionIcon(skill.id));assert.equal(new Set(icons).size,24);
...
test('client-side build validation exposes the same locked-level, budget and keystone rules shown in the UI',()=>{
  const level10={slots:['warrior-cleave','warrior-whirlwind','warrior-thrust','warrior-charge'],talents:{}};assert.deepEqual(parseSkillBuild(level10,'warrior',10),level10);
  assert.equal(parseSkillBuild({...level10,slots:['warrior-cleave','warrior-cleave',null,null]},'warrior',10),null,'duplicate skill');
  assert.equal(parseSkillBuild({...level10,slots:['warrior-banner',null,null,null]},'warrior',10),null,'locked skill');

...
test('schema 8 persists exact loadout, ranked talents, three presets and remaining cooldowns',{skip:!hasTestDatabase},async()=>{
 ...
 try{
  ...
  const loaded=await store.load(p.id,accountId);assert.equal(await store.schemaVersion(),9);assert.equal(loaded.revision,1);
...
test('schema5 migration adds legal level-based defaults without granting talents or resetting cooldowns',{skip:!hasTestDatabase},async()=>{
 ...
 try{
  ...
  assert.equal(await store.schemaVersion(),9);assert.deepEqual(restored.skillBuild,defaultSkillBuild('archer',14));assert.deepEqual(restored.skillPresets,[null,null,null]);assert.equal(restored.buildRevision,0);assert.deepEqual(restored.skillCooldowns,{'archer-rain':4.25});
...
test('schema 8 appends an empty RMB slot to schema 7 builds and presets without changing talents',{skip:!hasTestDatabase},async()=>{
 ...
 try{
  ...
  store=await openHeroStore({connectionString:db.url});const restored=(await store.load(p.id,accountId)).hero;
  assert.equal(await store.schemaVersion(),9);assert.deepEqual(restored.skillBuild,p.skillBuild);assert.deepEqual(restored.skillPresets,p.skillPresets);

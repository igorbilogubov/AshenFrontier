...
test('every catalogued clip exists as a compact ogg in the public tree',async()=>{
  ...
  for(const file of SOUND_FILES){
    ...
    assert(size>2000&&size<40000,`${file} ${size}`);
  }
...
test('world events pick a cue; unknown notices stay silent',()=>{
  ...
  assert.deepEqual([...SOUND_CUES.swing],['swing-1.ogg','swing-2.ogg','swing-3.ogg','whoosh.ogg']);
});

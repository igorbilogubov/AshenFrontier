...
async function connect(server,token){
  const client={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url}),states:[]};
  client.ws.on('message',data=>{const m=JSON.parse(data);if(m.type==='state'){client.state=m;client.states.push(m);}});
  await once(client.ws,'open');client.ws.send(JSON.stringify({type:'join',protocol:2,token}));await until(()=>client.state);return client;
}
...
test('real portal commands isolate regions and preserve hero identity, gear and position through PostgreSQL restart',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase(),token='9'.repeat(48),observerToken='8'.repeat(48),hero=newHero('Портал PostgreSQL'),observer=newHero('Лесной наблюдатель');
  hero.gold=123;await database.seed(token,persistentHero(hero));await database.seed(observerToken,persistentHero(observer));
  let server;const clients=[];
  try{
    server=await startTestServer(database);const owner=await connect(server,token),watcher=await connect(server,observerToken);clients.push(owner,watcher);
    ...
    assert.equal(owner.state.mobs.length,18);assert.equal(watcher.state.mobs.length,41);assert.equal(owner.state.self.gold,123);
    ...
    const saved=(await database.load(token)).hero;assert.deepEqual({x:saved.x,z:saved.z},arrival);assert.equal(saved.gold,123);assert.deepEqual(saved.items,hero.items);assert.deepEqual(saved.equipment,hero.equipment);
    server=await startTestServer(database);const restored=await connect(server,token);clients.push(restored);
    assert.equal(restored.state.self.id,hero.id);assert.equal(locationAt(restored.state.self),'stadium');assert.equal(restored.state.self.afk,null);assert.equal(restored.state.mobs.length,18);assert.equal(restored.state.self.gold,123);
    restored.ws.send(JSON.stringify({type:'portal',portalId:'stadium-camp'}));await until(()=>locationAt(restored.state.self)==='forest');assert.equal(restored.state.self.gold,123);assert.equal(restored.state.mobs.length,41);
...
test('network AFK chooses the real Stadium pen and cannot claim another region or grant forged rewards',{skip:!hasTestDatabase},async()=>{
  const database=await createTestDatabase(),token='7'.repeat(48),hero=newHero('Стадиум AFK','mage'),pen=STADIUM_PENS[0];Object.assign(hero,{x:pen.x,z:pen.z,level:20});await database.seed(token,persistentHero(hero));
  let server,client;
  try{
    server=await startTestServer(database);client=await connect(server,token);
    client.ws.send(JSON.stringify({type:'afk',enabled:true,spotId:'wolf-den',damage:999999,gold:999999}));
    await until(()=>client.states.some(state=>state.self.afk?.spotId===pen.id));assert.equal(client.state.self.gold,0);assert.equal(client.state.mobs.length,18);

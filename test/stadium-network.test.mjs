...
test('real portal commands isolate regions and preserve hero identity, gear and position through PostgreSQL restart',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    assert.equal(owner.state.mobs.length,24);assert.equal(watcher.state.mobs.length,71);assert.equal(owner.state.self.gold,123);
    ...
    restored.ws.send(JSON.stringify({type:'portal',portalId:'stadium-camp'}));await until(()=>locationAt(restored.state.self)==='forest');assert.equal(restored.state.self.gold,123);assert.equal(restored.state.mobs.length,71);

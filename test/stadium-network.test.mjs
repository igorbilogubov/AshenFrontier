...
test('real portal commands isolate regions and preserve hero identity, gear and position through PostgreSQL restart',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    assert.equal(owner.state.mobs.length,280);assert.equal(watcher.state.mobs.length,90);assert.equal(owner.state.self.gold,123);
    ...
    assert.equal(restored.state.self.id,hero.id);assert.equal(locationAt(restored.state.self),'stadium');assert.equal(restored.state.self.afk,null);assert.equal(restored.state.mobs.length,280);assert.equal(restored.state.self.gold,123);
...
test('network AFK chooses the real Stadium pen and cannot claim another region or grant forged rewards',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    await until(()=>client.states.some(state=>state.self.afk?.spotId===pen.id));assert.equal(client.state.self.gold,0);assert.equal(client.state.mobs.length,280);

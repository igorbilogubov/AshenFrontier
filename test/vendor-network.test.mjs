...
async function connect(server,fixture){
  ...
  const c={ws:new WebSocket(server.url.replace('http:','ws:')+'/ws',{origin:server.url,headers:login.headers}),state:null,events:[]};
...
test('real WebSocket vendor purchases and sales preserve the exact PostgreSQL economy across restart',{skip:!hasTestDatabase},async()=>{
  ...
  try{
    ...
    const fixture='e'.repeat(48);await database.seed(fixture,persistentHero(hero));
    server=await startTestServer(database,{dataDir});const c=await connect(server,fixture);clients.push(c);
    ...
    const stored=await database.load(fixture);assert.equal(stored.hero.gold,c.state.self.gold);assert.deepEqual(stored.hero.items.at(-1),bought);
    c.send({type:'sell',id:bought.id});await until(()=>c.state?.self.items.length===2);
    const expected=100-listing.price+sellPrice(bought);assert.equal(c.state.self.gold,expected);assert.equal((await database.load(fixture)).hero.gold,expected);
    ...
    const soldStorage=await database.load(fixture);assert.equal(soldStorage.hero.gold,boundExpected);assert(!soldStorage.hero.items.some(item=>item.id===starter.id));

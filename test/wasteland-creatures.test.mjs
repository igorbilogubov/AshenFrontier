...
const types=['ash-jackal','scorpion','monitor-lizard','scarab'],assets={};
const strides={'ash-jackal':[.72,1],scorpion:[.52,.76],'monitor-lizard':[.58,.82],scarab:[.54,.78]};
...
test('four original wasteland GLBs have complete skins, eight clips and bounded draw/asset budgets',()=>{
  for(const type of types){
    ...
    assert.equal(json.skins[0].joints.length,({scorpion:38,scarab:23,'monitor-lizard':20,'ash-jackal':24}[type]));
  }
});
test('wasteland locomotion keeps every support foot planted at the renderer stride',()=>{
  for(const type of types)for(const [clip,run] of [['Walk',0],['Run',1]]){
    const asset=assets[type],at=sample(asset,clip),spider=type!=='ash-jackal',golem=false;
    const prefixes=spider?Array.from({length:type==='scorpion'?8:type==='scarab'?6:4},(_,i)=>`Leg_${Math.floor(i/2)}_${i%2?'R':'L'}`):golem?['Leg_L','Leg_R']:['Front_L','Front_R','Hind_L','Hind_R'];
...
test('wasteland clips stay finite, close locomotion loops and settle at death',()=>{
  for(const type of types){
    ...
    assert(all.getSize(new T.Vector3()).length()<5.2,`${type}: stretching`);

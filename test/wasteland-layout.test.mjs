...
test('wasteland is large open territory with four species, packs, singles and independent IDs',()=>{
 ...
 assert.equal(SPAWNS.length,292);assert.equal(SPAWNS[193].eliteId,'glacier-warden');assert.equal(SPAWNS[290].eliteId,'obsidian-stinger');assert.equal(SPAWNS[291].eliteId,'sun-devourer');

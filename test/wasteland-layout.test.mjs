...
test('wasteland is large open territory with four species, packs, singles and independent IDs',()=>{
 ...
 assert.equal(SPAWNS.filter(s=>['forest','stadium','snow','wasteland'].includes(locationAt(s))).length,502);assert.equal(SPAWNS[193].eliteId,'glacier-warden');assert.equal(SPAWNS[290].eliteId,'obsidian-stinger');assert.equal(SPAWNS[291].eliteId,'sun-devourer');
